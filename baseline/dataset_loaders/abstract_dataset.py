...
class Dataset:
    ...
    def get_generator(self, batch_size=4, num_workers=4, pin_memory=True):
        generator_params = {'batch_size': batch_size,
                            ...
                            #'sampler': WeightedRandomSampler(weights = torch.ones(len(self)),num_samples=100),
                            'drop_last': True}

        return torch.utils.data.DataLoader(self, **generator_params,
                                           pin_memory=pin_memory)

    def get_generator_with_sampler(self,batch_size=4, num_workers=4, pin_memory=True,num_samples=300):
        generator_params = {'batch_size': batch_size,
                            #'shuffle': self.augment,
                            'num_workers': num_workers,
                            'sampler': DistributedSamplerWrapper(ReproducibleWeightedRandomSampler(weights = torch.ones(len(self)), num_samples=num_samples,seed=7230,replacement=False)),
                            'drop_last': True}
        
        print("Creating a sampled dataloader!")
        print(self)
        return torch.utils.data.DataLoader(self, **generator_params,
                                           pin_memory=pin_memory)
    ...
    def get_padded_tensor(self, numpy_waveform, start_index=0):
        ...
        if self.time_samples > 0:
            ...
            #    dtype=torch.float32)
            if type(numpy_waveform) == np.ndarray:
                tensor_wav = torch.tensor(
                numpy_waveform[rand_start:rand_start + self.time_samples],
                dtype=torch.float32)
            else:
                tensor_wav = numpy_waveform[rand_start:rand_start + self.time_samples].clone().detach()
