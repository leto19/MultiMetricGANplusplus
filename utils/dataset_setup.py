    ...
    for data_split in ['train', 'val', 'test']:
        ...
        for dataset_name in hparams[data_split]:
            ...
            if data_split == "train":
                ...
                # for the training set we return the dataset object NOT a generator such that
                # the brain class can make the sampled dataloader. 
                generators[this_dataset_name] = data_loader
                
                #generators[this_dataset_name] = data_loader.get_generator(
                #    batch_size=hparams['batch_size'], num_workers=hparams['n_jobs'])
                
                # generators[this_dataset_name] = data_loader.get_generator_with_sampler(
                #     batch_size=hparams['batch_size'], num_workers=hparams['n_jobs'],num_samples=hparams["number_of_samples"])
            else:
                if dataset_name == 'chime':
                    for fixed_n_sources in [1]:
                        data_loader = create_loader_for_simple_dataset(
                            ...
                            fixed_n_sources=fixed_n_sources,
                             et_only_active_speakers=False,
                            split=data_split,
                            n_samples=250)

                        this_dataset_name = f"{data_split}_{dataset_name}_{fixed_n_sources}sp"
                        generators[this_dataset_name] = data_loader.get_generator(
                            batch_size=hparams['val_batch_size'], num_workers=hparams['n_jobs'])
                else:
                    # non-chime
                    if data_split == "test":
                        data_loader = create_loader_for_simple_dataset(
                    ...
                    this_dataset_name = f"{data_split}_{dataset_name}_all"
                    generators[this_dataset_name] = data_loader.get_generator(
                        batch_size=hparams['val_batch_size'], num_workers=hparams['n_jobs'])
    ...
    for data_split in ['train', 'val', 'test']:
        ...
        for dataset_name in hparams[data_split]:
            if data_split == "train":
                ...
                # we let the brain class handle the generator so we get the random samples each epoch.
                generators[this_dataset_name] = data_loader
